...
void min_heap_insert(fila *f, int id, int chave, int ordem);
...
int main() {
    ...
    for (int i = 0; i < tam; i++) {
        int chave, id;
        scanf("%d %d", &chave, &id);
        min_heap_insert(f, id, chave, ordem_global++);
    }
    imprime_heap(f);
    ...
    for (int i = 0; i < m; i++) {
        ...
        min_heap_decrease(f, indice - 1, nova_chave);
    }
    imprime_heap(f);
...
int temPrioridade(no a, no b) {
    ...
    return a.ordem_insercao < b.ordem_insercao;
}
...
void min_heap_decrease(fila *f, int i, int nova_chave) {
    ...
    while (i > 0 && temPrioridade(f->array[i], f->array[parent(i)])) {
...
void min_heap_insert(fila *f, int id, int chave, int ordem) {
    ...
    f->array[i].ordem_insercao = ordem;
...
void min_heapfy(fila *f, int i) {
    ...
    int menor = i;
    if (l < f->tam_heap && temPrioridade(f->array[l], f->array[menor])) {
        menor = l;
    }
    if (r < f->tam_heap && temPrioridade(f->array[r], f->array[menor])) {
        menor = r;
    }
    if (menor != i) {
