...
//função para inserir numa lista circular os valores digitados pelo usuario no inicio
void insere_simbolo(Tambor *c, int valor) {
    No* p = c->cabeca;
            ...
            if (c->cabeca == NULL) { // se tambor não tiver nada logo nulo
                ...
                No* cauda = c->cabeca->prox;
                novono->prox = cauda;
                novono->ant = p;
                cauda-> ant = novono;
                c->cabeca->prox = novono;
            }
        }
            ...
            if (p->simbolo == cont) { //se achar o numero
                ...
                if (p->prox == p) { //se a lista só tiver 1 noh a cabeca recebe nulo
                    ...
                    if (p == l->cabeca) { //verifica se p é a cabeca
                        l->cabeca = p->ant; //se for cabeca recebe o ant de onde p tá apontando
                    }
                }
